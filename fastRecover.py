...
def quadSolveExpGrad_noLinesearch(y, x, eps, initialStepsize, recoveryLog, anchorsTimesAnchors=None): 
    (K,n) = x.shape
    ...
    print y.shape, x.transpose().shape
...
class myIterator:
    def __init__(self, Q, Q_A, anchors, recoveryLog, divergence, v_max, initial_stepsize, epsilon=10**(-7)):
        self.Q = Q
    ...
    def next(self):
        ...
        return (copy(Q.next()), copy(self.X), v, recoveryLog, anchors, divergence, self.anchorsTimesAnchors, self.initial_stepsize, self.epsilon)


def nonNegativeRecover(Q, Q_A, P_w, anchors, params, loss, initial_stepsize=1, epsilon=10**(-7)):
    outfile_name = params.log_prefix
    ...
    args = myIterator(Q, Q_A, anchors, outfile_name+".recoveryLog", divergence, V, initial_stepsize, epsilon)
    #rows = pool.imap_unordered(fastRecover, args, chunksize = 10)
    rows = (fastRecover(arg) for arg in args)
    print >>alphaLog, "\t".join(['word', 'iters', '[alpha_0, ..., alpha_k]', 'gap', 'objective'])
    for r in rows:
        v, it, obj, alpha, stepsize, t, gap = r
        A[v, :] = alpha
        if v % 1 == 0:
            print >>alphaLog, "\t".join([str(x) for x in [v, it, alpha, gap, obj]])
            alphaLog.flush()
            sys.stdout.flush()
